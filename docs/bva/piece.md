...
## Method under test: `getLegalMoves(Board board)`
`getLegalMoves(Board board)` is abstract in `Piece`, so concrete movement behavior is determined in the specific subclass BVA and tests.